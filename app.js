    ...
    apps: [
      {
        ...
        title: 'GitHub',
        desc: 'Code repository and version control',
        color: '#ffffff',
        icon: 'fa-github',
        colspan: 1,
        rowspan: 1,
      ...
      },
      {
        ...
        desc: 'Developer community and Q&A',
        color: '#f48024',
      ...
      },
      {
        ...
        title: 'ChatGPT',
        desc: 'AI assistant and coding companion',
        color: '#10a37f',
        icon: 'fa-robot',
        colspan: 1,
        rowspan: 1,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const storedWorkMode = localStorage.getItem('launchpad_work_mode_active');
  workModeActive = storedWorkMode === null ? true : storedWorkMode === 'true';
  ...
  groups.forEach((group, groupIndex) => {
    ...
    if (editModeActive) {
      ...
      header.querySelector('.delete-group').addEventListener('click', async () => {
        if (await showCustomConfirm(`Are you sure you want to delete "${group.title}" and all its apps?`, { title: 'Delete Category' })) {
  ...
  group.apps.forEach((app, appIndex) => {
    ...
    if (editModeActive) {
      ...
      deleteBadge.addEventListener('click', async (e) => {
        e.stopPropagation();
        if (await showCustomConfirm(`Delete app "${app.title}"?`, { title: 'Delete Application' })) {
...
function querySpotlight(query) {
  ...
  const commands = [
    { type: 'cmd', title: '/work', desc: 'Toggle Microsoft Work Mode', icon: 'fa-briefcase', action: () => { const t = document.getElementById('work-mode-toggle'); if(t) { t.checked = !t.checked; t.dispatchEvent(new Event('change')); } } },
...
function initWorkMode() {
  const workToggle = document.getElementById('work-mode-toggle');
  ...
  const applyWorkModeUI = (active) => {
    ...
    if (workToggle.checked !== active) {
      workToggle.checked = active;
    }
    ...
    if (mainGrid) {
      ...
      setTimeout(() => {
        ...
        // Clear inline styles after transition to avoid permanent stacking context
        setTimeout(() => {
          mainGrid.style.opacity = '';
  ...
  workToggle.addEventListener('change', (e) => {
    applyWorkModeUI(e.target.checked);
  });

  if (workModeActive) {
    applyWorkModeUI(true);
  } else {
    applyWorkModeUI(false);
  }
}
  ...
  activeWidgets.forEach((widget, index) => {
    ...
    if (editModeActive) {
      ...
      deleteBadge.addEventListener('click', async (e) => {
        e.stopPropagation();
        e.preventDefault();
        if (await showCustomConfirm(`Are you sure you want to delete widget "${widget.title}"?`, { title: 'Delete Widget' })) {
  ...
  bodyEl.innerHTML = `
    <div class="m365-detail-popup-content" style="padding: 20px; color: var(--text-primary);">
      ...
      <div style="display:flex; justify-content:flex-end;">
        <button class="btn-macos btn-macos-secondary" onclick="closeAppDetailOverlay()">Close</button>
      </div>
    </div>
  `;
...
function renderTorrentRows(torrents, container, isMock) {
  ...
  container.querySelectorAll('.delete-torrent-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (!await showCustomConfirm("Are you sure you want to remove this torrent?", { title: 'Remove Torrent', okText: 'Remove' })) return;
      const deleteFiles = await showCustomConfirm("Do you also want to DELETE the downloaded files from disk?", { 
        title: 'Delete Data', 
        okText: 'Delete Data',
        okColor: '#ff3c00'
      });
...
function showCustomConfirm(message, options = {}) {
  return new Promise((resolve) => {
    ...
    if (options.okColor) {
      okBtn.style.background = options.okColor;
      okBtn.style.borderColor = 'rgba(255, 255, 255, 0.15)';
    } else {
      okBtn.style.background = '#ff3c00'; // default warning red
      okBtn.style.borderColor = 'rgba(255, 60, 0, 0.3)';
    }
